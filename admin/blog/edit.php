...
$id = $_GET['id'] ?? 0;
if (!$id) {
    header('Location: ' . SITE_URL . '/admin/blog/manage.php');
    exit();
}
                ...
                <form method="POST" enctype="multipart/form-data" class="space-y-6">
                    <div class="bg-white shadow rounded-lg p-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea name="content" rows="10"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500"><?php echo htmlspecialchars($post['content']); ?></textarea>
                                    </div>
                                    <div>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" name="title_fr"
                                            value="<?php echo htmlspecialchars($post['title_fr']); ?>"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Contenu</label>
                                        <textarea name="content_fr" rows="10"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500"><?php echo htmlspecialchars($post['content_fr']); ?></textarea>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Description Meta</label>
                                        <input type="text" name="meta_description_fr"
                                            value="<?php echo htmlspecialchars($post['meta_description_fr'] ?? ''); ?>"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="status"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
                                    <option value="draft"
                                        <?php echo ($post['status'] ?? 'draft') === 'draft' ? 'selected' : ''; ?>>Draft
                                    </option>
                                    <option value="published"
                                        <?php echo ($post['status'] ?? '') === 'published' ? 'selected' : ''; ?>>
                                        Published</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <div class="mt-2">
                                <img src="<?php echo SITE_URL . $post['image_path']; ?>" alt="Current featured image"
